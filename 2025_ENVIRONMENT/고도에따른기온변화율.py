...
# 기온 변화율 (절댓값)
dT_dh_grass = np.abs(np.gradient(T_grass_profile, h_vals))
dT_dh_sand = np.abs(np.gradient(T_sand_profile, h_vals))

# 그래프 그리기
plt.figure(figsize=(8, 6))
plt.plot(h_vals, dT_dh_grass, label='인조 잔디', color='green')
plt.plot(h_vals, dT_dh_sand, label='모래', color='saddlebrown')

plt.xlabel('고도 (m)')
plt.ylabel('기온 변화율 |dT/dh| (°C/m)')
plt.title('고도에 따른 기온 변화율 크기')
plt.grid(True)
plt.legend()
plt.tight_layout()
plt.show()
